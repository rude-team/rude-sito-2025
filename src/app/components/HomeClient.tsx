...
interface HomeClientProps {
  leftText: string
  rightText: string
  imageUrl: string | null   // URL CDN Sanity oppure null → usa /lapide.png locale
  videoUrl: string
}

const FALLBACK_VIDEO = 'https://vimeo.com/1138108627?fl=ip&fe=ec'
const FALLBACK_IMAGE = '/lapide.png'

export default function HomeClient({ leftText, rightText, imageUrl, videoUrl }: HomeClientProps) {
  const [isModalOpen, setIsModalOpen] = useState(false)

  const resolvedImage = imageUrl ?? FALLBACK_IMAGE
  const resolvedVideo = videoUrl || FALLBACK_VIDEO
          ...
          <span className="flex justify-center">
            ...
            >
              <Image
                ...
                unoptimized={resolvedImage === FALLBACK_IMAGE}
              />
            </button>
          </span>
